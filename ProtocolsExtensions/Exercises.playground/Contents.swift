...
class Teenager: Babysitter {
    ...
    func playCandyland(_ numberOfTimes: Int) {
        print("Some game stuff \(numberOfTimes) times")
    }
    
    func read(_ book: String, firstLine: String, asleep: Bool) -> Bool {
        if asleep {
            print("We read \(book), and after line \"\(firstLine)\" it felt asleep")
            return true
        } else {
            print("We read \(book), and after line \"\(firstLine)\" line it didn't fall asleep")
            return false
        }
    }
}
...
let boy = Teenager(age: 13, responsible: false, patience: Level.low)
boy.playCandyland(10)
boy.read("Cormen: Introduction to Algorithms", firstLine: "Hey guys! Thanks for tuning in to another...", asleep: true)
//: __Problem 2__
//:
...
class Animal: Adorable {
    ...
    func frolick() {
        print("Some joyfull stuff lets finish this i wanna sleeep pleeeaaaseeee")
    }
...
let animal = Animal(species: "cat", numberOfLegs: 4)
animal.frolick()
...
extension UIColor
{
    static func get_pistachio() -> UIColor {
