...
let reversed_surnames = surnames.sorted(by: {
    $1 < $0
})
...
let reversed_averages = battingAverages.sorted(by: {
    $1 < $0
})
...
let divisible_by_twelve = numbers.filter({
    $0 % 12 == 0
})

//: __3b.__
//: Rewrite the filtering closure expression to be as concise as possible.
let divisible_by_twelve_again = numbers.filter({
    $0 % 12 == 0
})
//: __Problem 4__
//:
...
let new_array = particleSizesInMicrons.filter({
    $0 < 20
})
...
let rup_to_dollar_exchange_rate = 2 // it's not 2 but I don't care
let prices_in_dollars = pricesInRupees.map({
    $0 * rup_to_dollar_exchange_rate
})
...
let time_since_graduation = 13

let goalTimes = oldTimes.map(){(time: String) -> String in
    var seconds = timeIntervalFromString(time)
    seconds += 13
    let res = timeStringFromInterval(seconds)
    return String(res)
}
